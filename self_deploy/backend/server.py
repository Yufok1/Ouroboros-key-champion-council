        ...
        for i, ch in enumerate(text):
            ...
            if ch == '{':
                ...
                if depth == 0 and start >= 0:
                    try:
                        parsed = json.loads(text[start:i+1]); break
                    except (json.JSONDecodeError, TypeError):
                        start = -1
    ...
    async def _stream():
        ...
        for iteration in range(max_iterations):
            ...
            invoke_result = await _call_tool("invoke_slot", {
                "slot": slot, "text": message, "mode": "generate",
            ...
            if not raw_output or "error" in invoke_result:
                yield f"event: error\ndata: {json.dumps({'error': str(invoke_result.get('error', 'Empty')), 'iteration': iteration})}\n\n"
                break
            try:
            ...
            if not directive:
                yield f"event: final\ndata: {json.dumps({'final_answer': model_output[:3000], 'iterations': iteration+1, 'tool_calls_total': total_tool_calls, 'note': 'raw output'})}\n\n"
                break
            ...
            if len(calls) > 8: calls = calls[:8]
            if directive.get("reasoning"):
                yield f"event: step\ndata: {json.dumps({'iteration': iteration, 'type': 'reasoning', 'reasoning': directive['reasoning']})}\n\n"
            batch_results = []
            for ci, call in enumerate(calls):
                tool_name, tool_args = call["tool"], call.get("args", {})
                if tool_name in _AGENT_LOOP_BLOCKED_TOOLS:
                    batch_results.append(f"{ci+1}. {tool_name} [BLOCKED]")
                    yield f"event: tool_call\ndata: {json.dumps({'tool': tool_name, 'status': 'blocked', 'iteration': iteration})}\n\n"
                    continue
                ...
                try:
                    ...
                    call_ms = int((time.time() - call_start) * 1000)
                    result_text = f"EXCEPTION: {e}"; status = "error"
                ...
                total_tool_calls += 1
                ctx = result_text[:1500] + "\n…[truncated]" if len(result_text) > 1500 else result_text
                batch_results.append(f"{ci+1}. {tool_name} [{status.upper()}]: {ctx}")
                all_traces.append({"tool": tool_name, "status": status, "duration_ms": call_ms, "iteration": iteration})
            ...
            yield f"event: final\ndata: {json.dumps({'final_answer': f'Exhausted {max_iterations} iterations. Made {total_tool_calls} tool calls.', 'iterations': max_iterations, 'tool_calls_total': total_tool_calls})}\n\n"
